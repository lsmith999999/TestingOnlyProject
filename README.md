...
<table>
  ...
  <tbody>
    <tr>
      <td valign="top"><b><a href="#declare_class_has_function">DECLARE_CLASS_HAS_FUNCTION(NAME)</a></b><br><br>Use when you need to check a class for an <i>exact</i> type match of the <i>non-static</i> (public) member function given by the <i>NAME</i> (macro) arg. Just pass the target class as the 1st arg to the templates created by this macro (see next column), and the function type itself as the 2nd arg, such as:<br><br><pre>int (std::string_view, float) const noexcept</pre>The function you're targeting (<i>NAME</i>) must have this <i>exact</i> function type for a match to occur. See <i>Example</i> column for details.</td>
      <td valign="top"><b><a href="#declare_class_has_function">DECLARE_CLASS_HAS_FUNCTION(NAME)</a> </b>declares:<pre><b>template &lt;IS_CLASS_C T, IS_FUNCTION_C U&gt;<br>struct ClassHasFunction_##NAME : std::bool_constant&lt&gt;<br>{<br>};<br><br>template &lt;IS_CLASS_C T, IS_FUNCTION_C U&gt;<br>inline constexpr bool ClassHasFunction_##NAME##_v;</b></pre>First template above inherits from "<i>std::true_type</i>" or "<i>std::false_type</i>" accordingly (based on whether the <i>non-static</i> (public) member function given by the macro's NAME arg exists in the class/struct passed as the template's 1st arg, and exactly matches the function type given by the 2nd arg). Second template above is just a bool helper variable you'll usually rely on instead (that defers to the first template). See <i>Example</i> column for details.</td>
    ...
    <tr>
      <td valign="top"><b><a href="#declare_class_has_non_overloaded_static_function_traits">DECLARE_CLASS_HAS_NON_OVERLOADED_STATIC_FUNCTION_TRAITS(NAME)</a></b><br><br>Identical to macro just above but targets a <i>static</i> function instead of non-static. See macro just above for details.</td>
      <td valign="top"><b><a href="#declare_class_has_non_overloaded_static_function_traits">DECLARE_CLASS_HAS_NON_OVERLOADED_STATIC_FUNCTION_TRAITS(NAME)</a> </b>declares:<pre><b>template &lt;IS_CLASS_C T, template &lt;TRAITS_FUNCTION_C&gt; class HasFunctionTraitsT&gt;<br>struct ClassHasNonOverloadedStaticFunctionTraits_##NAME : std::bool_constant&lt&gt;<br>{<br>};<br><br>template &lt;IS_CLASS_C T, template &lt;TRAITS_FUNCTION_C&gt; class HasFunctionTraitsT&gt;<br>inline constexpr bool ClassHasNonOverloadedStaticFunctionTraits_##NAME##_v;</b></pre>Functionally equivalent to templates on the row just above but targets a <i>static</i> function instead of non-static. See templates on row just above for details.</td>
...
<ins>IMPORTANT</ins>:<br /> Note that this template targets non-variadic arguments in "*F*" only. If "*F*" is a variadic function that is (i.e., [IsVariadic_v](#isvariadic_v) returns true), then the template completely ignores this (the function's variadic arguments are not checked against "*ArgsT*"). Caution advised.</details></blockquote>
...
Identical to [AreArgTypesSame_v](#areargtypessame_v) just above except the argument list is passed as a [std::tuple](https://en.cppreference.com/w/cpp/utility/tuple) instead of a parameter pack (via the 2nd template arg). The types in the [std::tuple](https://en.cppreference.com/w/cpp/utility/tuple) are therefore used for the resulting argument list. "*AreArgTypesSameTuple_v*" is otherwise identical to [AreArgTypesSame_v](#areargtypessame_v). See this for details.</details></blockquote></details></blockquote>
...
This macro and its cousins below declare templates allowing you to determine if an arbitrary class/struct has a (public) member function with a given name and function type. For a comparative summary of these macros and the templates they create, please see [Determining if a member function exists](#determiningifamemberfunctionexists). The *DECLARE_CLASS_HAS_FUNCTION* macro specifically targets *non-static* (public) member functions only, without regard to whether they're overloaded or not (the other macros below provide slightly different flavors to target *static* member functions and/or non-overloaded functions). *DECLARE_CLASS_HAS_FUNCTION* allows you to do this by creating templates taking the class/struct type you're targeting as the 1st template arg, and the *non-static* function type itself as the 2nd template arg, such as ```int (std::string_view, float) const noexcept```. The template will then determine if the passed class/struct (the 1st template arg) has a *non-static* (public) member function that exactly matches the latter type (the 2nd template arg). Note that the function name itself, "*Whatever*" in the latter case, is passed to the macro itself and embedded in the template's name. The templates created by the macro then always target functions with that specific name only. The *DECLARE_CLASS_HAS_FUNCTION* macro itself is used to declare template "*ClassHasFunction_\#\#NAME*" which is used to determine if any class or struct has a *non-static* (public) member function with the name given by *NAME* (as passed to this macro). The macro simply declares this template and its "*\_v*" (*bool*) helper template you'll normally use instead. Just pass the macro the name of the *non-static* member function you're interested in via the macro's NAME arg like so (passing function name "*Whatever*" in this example). You'll always declare this at namespace scope:

```C++
DECLARE_CLASS_HAS_FUNCTION(Whatever)
```

This declares the following two templates used to check for the (public) function "*Whatever*", where the name of each template always starts with "*ClassHasFunction_*" followed by the *NAME* arg passed to the macro (representing the name of the *non-static* (public) function you're targeting, "*Whatever*" in this case). Note that the resulting "*ClassHasFunction_Whatever*" template always inherits from "*std::true_type*" or "*std::false_type*" as described below (which are just [std::bool_constant](https://en.cppreference.com/w/cpp/types/integral_constant) specializations). The 2nd template below is just a helper (bool) variable template with the usual "*_v*" suffix that defers to the 1st template in the usual C++ way:
...
These templates (you'll usually use the "*_v*" version just above) can then be used to determine if any class or struct has a *non-static* (public) function called "*Whatever*" with the exact function type you're interested in. The "*ClassHasFunction_Whatever*" template above always inherits from "*std::true_type*" if so or "*std::false_type*" otherwise (which are just specializations of "*std::bool_constant*" seen in its declaration above - its template arg is omitted in the declration  above however since it's an implementation detail handled by the library). Again, the second template above is just the "*bool*" equivalent.
