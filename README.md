...
The following provides a complete (alphabetical) list of all helper templates. Two separate tables exist, the first for read traits allowing you to read any part up a function's type, and the second for write traits, allowing you to update any part of a function's type. Note that the first template arg of every template is the function you're targeting, whose name is always "F" (a function type). Again, "F" supports the actual C++ function type (always refers to a free or static member function), pointers to functions, references to functions (note however that references to non-static member functions aren't supported by C++ itself), references to pointers to functions, and functors including lambdas. IMPORTANT: For functors please note that all traits implicitly refer to the non-static member function "F::operator()" unless noted otherwise (so if "F" is the type of a lambda for instance then the traits target "operator()" of the compiler-generated class for that lambda). When referring to "F" in the "Description" column for each trait, it should therefore be understood that the trait usually applies to "F::operator()" whenever "F" itself is a functor (and when referring to "F" in the description itself, it's therefore usually referring to "F::operator()").
