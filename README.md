...
## Determining if a non-overloaded, non-static member function exists

Determining if a *non-overloaded*, non-static member function with a specific signature exists in an arbitrary class is a very common requirement for many users. The library provides a very clean way to do this using two similar techniques. Before describing this however, please note that you cannot determine if an *overloaded* function with a specific signature exists using this library because C++ itself provides no way to do so. There is nothing in the language that reliably supports it at this writing so if the function you wish to target might be overloaded then you'll have to wait until C++ does support it (and this will presumably be available in the not-too-distant future since reflection is scheduled for C++26). Note that more about this situation (why it can't be implemented) is discussed under [DECLARE_HAS_NON_OVERLOADED_MEMBER_FUNCTION](#declare_has_non_overloaded_member_function).

For now, if you wish to determine whether a function, say, ```int Whatever(std::string_view, float) const noexcept``` exists in an arbitrary class, then you'll rely on either the macro [DECLARE_HAS_NON_OVERLOADED_MEMBER_FUNCTION](#declare_has_non_overloaded_member_function) and/or [DECLARE_HAS_NON_OVERLOADED_MEMBER_FUNCTION_TRAITS](#declare_has_non_overloaded_member_function_traits) to do so. Each simply declares appropriate templates you'll rely on to check for the particular function you're interested in, such as "*Whatever*" above. These macros are briefly described in the table just below with examples in the last column (note that the table is somewhat lengthy so you may need to rely on the horizontal scrollbar beneath it if required). For now the following table provides a brief introduction only. Most users can start using these macros after reading the examples in this table. For complete details on each macro, see [DECLARE_HAS_NON_OVERLOADED_MEMBER_FUNCTION](#declare_has_non_overloaded_member_function) and [DECLARE_HAS_NON_OVERLOADED_MEMBER_FUNCTION_TRAITS](#declare_has_non_overloaded_member_function_traits).

### Table highlighting how to use macros described above

<table>
  <thead>
    <tr>
      <th>Macro</th>
      <th>Templates created by this macro (implementation details omitted or obscured)</th>
  ...
  <tbody>
    <tr>
      <td valign="top"><i><b><a href="#declare_has_non_overloaded_member_function">DECLARE_HAS_NON_OVERLOADED_MEMBER_FUNCTION(NAME)</i></b><br><br>Use when you need to check for an <i>exact</i> type match of the function given by the <i>NAME</i> (macro) arg). Just pass the target class as the 1st arg to the templates created by this macro (see next column), and the function type itself as the 2nd arg, such as:<br><br><pre>int (std::string_view, float) const noexcept</pre>The function you're targeting must have this <i>exact</i> function type for a match to occur. See <i>Example</i> column for details.</td>
      <td valign="top"><pre>template &lt;IS_CLASS_C T, IS_FUNCTION_C U&gt;<br>struct HasNonOverloadedMemberFunction_##NAME : std::bool_constant&lt&gt;;<br>{<br>};<br><br>template &lt;IS_CLASS_C T, IS_FUNCTION_C U&gt;<br>inline constexpr bool HasNonOverloadedMemberFunction_##NAME##_v;</pre>First template above inherits from "<i>std::true_type</i>" or "<i>std::false_type</i>" accordingly (if the function given by the macro's NAME arg exists in the class/struct passed as the template's 1st arg, and exactly matches the function type given by the 2nd arg). Second template is just a bool helper variable you'll usually rely on instead (that defers to the first struct). See <i>Example</i> column for details.</td>
      <td><details><summary>Click here to open/close example 👀</summary><pre>// Standard C/C++ headers<br>#include &lt;string_view&gt;<br><br>// Only file in this repository you need to explicitly #include<br>#include &quot;TypeTraits.h&quot;<br><br>// Everything in the library declared in this namespace<br>using namespace StdExt;<br><br>////////////////////////////////////////////////////////<br>// Class you wish to check for non-overloaded function<br>// <i>&quot;Whatever&quot;</i> with the exact signature seen below<br>////////////////////////////////////////////////////////<br>class Test<br>{<br>public:<br>    int Whatever(std::string_view, float) const noexcept;<br>};<br><br>//////////////////////////////////////////////////////////////////<br>// Declares template <i>&quot;HasNonOverloadedMemberFunction_Whatever_v&quot;</i><br>// called below which is set to true if the (non-overloaded)<br>// function <i>&quot;Whatever&quot;</i> exists in the class you pass it (via the<br>// 1st arg), exactly matching the function type you pass it (via<br>// the 2nd arg), or false otherwise<br>//////////////////////////////////////////////////////////////////<br>DECLARE_HAS_NON_OVERLOADED_MEMBER_FUNCTION(Whatever)<br><br>int main()<br>{<br>    ////////////////////////////////////////////////////////////<br>    // Returns true since class <i>&quot;Test&quot;</i> (the 1st template arg)<br>    // has a non-static member function called <i>&quot;Whatever&quot;</i> with<br>    // the exact type being passed via the 2nd template arg,<br>    // <i>&quot;int (std::string_view, float) const noexcept&quot;</i> in this<br>    // case.<br>    ////////////////////////////////////////////////////////////<br>    constexpr bool hasNonOverloadedMemberFunction_Whatever = HasNonOverloadedMemberFunction_Whatever_v&lt;Test, int (std::string_view, float) const noexcept&gt;;<br><br>    return 0;<br>}</pre></details></td>
    </tr>
    <tr>
      <td valign="top"><i><b><a href="#declare_has_non_overloaded_member_function_traits">DECLARE_HAS_NON_OVERLOADED_MEMBER_FUNCTION_TRAITS(NAME)</i></b><br><br>Use when the above technique doesn't suffice because you need to do a more granular check for the function (say, if you want to ignore the function's return type, its <i>noexcept</i> specification, check a specific arg type only, etc.). You have complete control over what to check for opposed to the above technique which always does an exact match of the function type you pass (so instead of passing a function type, you'll effectively pass the exact traits you wish to check). See the <i>Example</i> column for details.</td>
      <td valign="top"><pre>template &lt;IS_CLASS_C T, template &lt;TRAITS_FUNCTION_C&gt; class HasFunctionTraitsT&gt;<br>struct HasNonOverloadedMemberFunctionTraits_##NAME : std::bool_constant&lt&gt;;<br>{<br>};<br><br>template &lt;IS_CLASS_C T, template &lt;TRAITS_FUNCTION_C&gt; class HasFunctionTraitsT&gt;<br>inline constexpr bool HasNonOverloadedMemberFunctionTraits_##NAME##_v;</pre>Similar to the templates above except the 2nd template arg is no longer the function's type you wish to match, but a customized class template you write instead. Your template is passed the function's type as its only template arg, and it always inherits from <i>"std::bool_constant"</i> which you'll specialize on true or false to indicate if the function matches your requirements (by testing the function's traits using the templates in this library). See <i>Example</i> column for details.</td>
      <td><details><summary>Click here to open/close example 👀</summary><pre>// Standard C/C++ headers<br>#include &lt;string_view&gt;<br><br>// Only file in this repository you need to explicitly #include<br>#include &quot;TypeTraits.h&quot;<br><br>// Everything in the library declared in this namespace<br>using namespace StdExt;<br><br>////////////////////////////////////////////////////////<br>// Class you wish to check for non-overloaded function<br>// <i>&quot;Whatever&quot;</i> based on the particular traits seen below<br>////////////////////////////////////////////////////////<br>class Test<br>{<br>public:<br>    int Whatever(std::string_view, float) const noexcept;<br>};<br><br>////////////////////////////////////////////////////////////////////////<br>// Declares template <i>&quot;HasNonOverloadedMemberFunctionTraits_Whatever_v&quot;</i><br>// called below which is set to true if the (non-overloaded) function<br>// <i>&quot;Whatever&quot;</i> exists in the class you pass it (1st arg), and has the <br>// traits passed via the 2nd arg (<i>"HasAllTraits"</i> declared just below <br>// in this example). Variable set to false otherwise.<br>////////////////////////////////////////////////////////////////////////<br>DECLARE_HAS_NON_OVERLOADED_MEMBER_FUNCTION_TRAITS(Whatever)<br><br>////////////////////////////////////////////////////////////////////////<br>// The following struct creates the condition used to determine if<br>// function <i>&quot;Whatever&quot;</i> exists in the call just below (by<br>// testing the specific traits you're interested in). The following<br>// call effectively performs the equivalent of the example above but<br>// does so by checking the individual traits instead (every trait<br>// comprising a function is accounted for in this example). The above<br>// example would therefore normally be more suitable in this<br>// particular case since its interface is more natural and less<br>// verbose. However, if you need to customize your check then you can<br>// rely on the following technique instead and just modify it for<br>// your particular needs (say, if you don't want to check the return<br>// type for instance - you can just remove the call to<br>// "HasReturnType_v" seen below). The following technique therefore<br>// gives you complete granular control over what to check compared to<br>// the technique used in the example above. For further details on<br>// the following struct <a href="#declare_has_non_overloaded_member_function_traits">DECLARE_HAS_NON_OVERLOADED_MEMBER_FUNCTION_TRAITS</a><br>// <br>//////////////////////////////////////////////////////////////////////////<br>template &lt;TRAITS_FUNCTION_C F&gt;<br>struct HasAllTraits : std::bool_constant&lt;HasReturnType_v&lt;F, int&gt; && // <b><i>Must have "int" return type</i></b><br>                                         CallingConvention_v&lt;F&gt == DefaultCallingConvention_v&lt;false&gt; && // <b><i>Must have the default calling convention</i></b><br>                                         HasArgTypes_v&lt;F, std::string_view, float&gt; && // <b><i>Must have 2 args "std::string_view" and "float"</i></b><br>                                         !IsVariadic_v&lt;F&gt; && // <b><i>Must not be variadic (can't end with "...")</i></b><br>                                         IsMemberFunctionConst_v&lt;F&gt; && // <b><i>Must be "const"</i></b><br>                                         !IsMemberFunctionVolatile_v&lt;F&gt; && // <b><i>Must not be "volatile"</i></b><br>                                         MemberFunctionRefQualifier_v&lt;F&gt; == RefQualifier::None && // <b><i>Must not have any ref-qualifiers ("&" or "&&" though rare in practice)</i></b><br>                                         IsNoexcept_v&lt;F&gt; // <b><i>Must be "noexcept"</i></b><br>                                        ><br>{<br>};<br><br>int main()<br>{<br>    ///////////////////////////////////////////////////////////<br>    // Returns true since class <i>&quot;Test&quot;</i> (the 1st template arg)<br>    // has a non-overloaded, non-static member function called<br>    // <i>&quot;Whatever&quot;</i> matching the condition just above (passed<br>    // as the 2nd template arg in the following call):<br>    ///////////////////////////////////////////////////////////<br>    constexpr bool hasNonOverloadedMemberFunction_Whatever = HasNonOverloadedMemberFunctionTraits_Whatever_v&lt;Test, HasAllTraits&gt;;<br><br>    return 0;<br>}</pre></details></td>
    </tr>
  </tbody>
</table>
...
<a name="DECLARE_HAS_NON_OVERLOADED_MEMBER_FUNCTION"></a><details><summary>DECLARE_HAS_NON_OVERLOADED_MEMBER_FUNCTION</summary>
```C++
DECLARE_HAS_NON_OVERLOADED_MEMBER_FUNCTION(NAME)
```
This macro and its cousin [DECLARE_HAS_NON_OVERLOADED_MEMBER_FUNCTION_TRAITS](#declare_has_non_overloaded_member_function_traits) just below declare templates allowing you to determine if an arbitrary class/struct has a non-overloaded, non-static member function with a given name and function type (signature). For a comparative summary of these two macros and the templates they create, please see [here](#determiningifamemberfunctionexists). The DECLARE_HAS_NON_OVERLOADED_MEMBER_FUNCTION macro allows you do to this by creating templates taking the class/struct type you're targeting as the 1st template arg, and the function type (signature) itself as the 2nd argument, such as "*int (float, std::string_view") const noexcept*". The template will then determine if the passed class/struct (your 1st arg) has a non-overloaded, non-static member function that exactly matches the latter type (your 2nd arg). Note that the function name itself, "*Whatever*" in the latter case, is passed to the macro itself. The templates created by the macro always target functions with that specific name (more on this shortly). Note that if you don't wish for an exact match however (when you use these templates), say, for instance, if you only want to check that the args of function "*Whatever*" are "*(std::string_view, float)*" but don't care about any other component of the function (its return type, whether it's "*const*", "*noexcept*", etc.), then [DECLARE_HAS_NON_OVERLOADED_MEMBER_FUNCTION_TRAITS](#declare_has_non_overloded_member_function_traits) should be used instead. It performs the same task as DECLARE_HAS_NON_OVERLOADED_MEMBER_FUNCTION but the templates it creates provide precise control over the matching process (at the expense of more verbose syntax though still very clean). You can therefore customize the check for the function you're interested in using any criteria you require (so you have complete flexibility to check anything you want). The following macro however, DECLARE_HAS_NON_OVERLOADED_MEMBER_FUNCTION, creates templates that always perform an *exact* match of all the function's components instead (return type, arg types, *noexcept*, *const*, etc.), but its syntax provides a cleaner and more natural interface than DECLARE_HAS_NON_OVERLOADED_MEMBER_FUNCTION_TRAITS](#declare_has_non_overloaded_member_function_traits) (which is capable of performing the same task as DECLARE_HAS_NON_OVERLOADED_MEMBER_FUNCTION but again, with more verbose syntax).

The DECLARE_HAS_NON_OVERLOADED_MEMBER_FUNCTION macro is used to declare helper template "*HasNonOverloadedMemberFunction_NAME* which is used to determine if any class or struct has a non-overloaded, non-static member function with the name given by *NAME* (as passed to this macro). The macro simply declares this template. Just pass it the name of the non-static member function you're interested in via the macro's NAME arg like so (passing function name "*Whatever*" in this example). You'll declare this at namespace scope:

    DECLARE_HAS_NON_OVERLOADED_MEMBER_FUNCTION(Whatever)

This declares the following two templates used to check for the function "*Whatever*", where the name of each template always starts with "*HasNonOverloadedMemberFunction_*" followed by the *NAME* arg passed to the macro (representing the name of the member function you're targeting, "*Whatever*" in this case). Note that while not shown in the following declaration (since the implementations details aren't relevant), the "*HasNonOverloadedMemberFunction_Whatever*" template always inherits from "*std::true_type*" or "*std::false_type*" as described below. The 2nd template is just a helper (bool) variable template with the usual "*_v*" suffix that defers to the 1st template in the usual C++ way:

    template<IS_CLASS_C T, IS_FUNCTION_C U>
    struct HasNonOverloadedMemberFunction_Whatever; // Inherits from "std::true_type" or "std::false_type" as described below

    template<IS_CLASS_C T, IS_FUNCTION_C U>
    constexpr bool HasNonOverloadedMemberFunction_Whatever_v = HasNonOverloadedMemberFunction_Whatever<T, U>::value;

These templates (you'll usually the "*_v*" version just above) can then be used to determine if any class or struct has a non-overloaded member function called "*Whatever*" with the exact function type you're interested in. The "*HasNonOverloadedMemberFunction_Whatever*" template above always inherits from "*std::true_type*" if so or "*std::false_type*" otherwise (and this is constrained by a concept in C++20 or later). Again, the second template is just the "*bool*" equivalent.

For instance, after calling this macro via DECLARE_HAS_NON_OVERLOADED_MEMBER_FUNCTION(Whatever) seen above, you can then do the following.

Given any class or struct with a non-static member function called "*Whatever*":
    ...
    // arg) has a non-static member function called
    // "Whatever" with the exact type being passed via the
    // 2nd template arg ("int (float) const noexcept" in
    // this case):
    ////////////////////////////////////////////////////////
    constexpr bool hasNonOverloadedMemberFunction_Whatever = HasNonOverloadedMemberFunction_Whatever_v<Test, int (std::string_view, float) const noexcept>;

The above variable "*hasNonOverloadedMemberFunction_Whatever*" will be set to true because class "*Test*" (the 1st template arg passed in the above call) has a non-static member function called "*Whatever*" (the *NAME* arg passed to this macro), whose type exactly matches the 2nd template arg that was passed. Note that the 2nd template arg is the native C++ function type you're interested in, so "*int (std::string_view, float) const noexcept*" in this case (exactly matching the type of function "*Test::Whatever*"). It's therefore not a function pointer but the actual C++ function type itself (is meets the criteria of [std::is_function](https://en.cppreference.com/w/cpp/types/is_function)). Note that "*true*" is returned only if the class or struct you pass for the 1st template arg has a non-static member function named "*Whatever*" with this exact function type. To this end, note that function types in C++ are always of the following general form, where items in square brackets are optional (and again, this is what [std::is_function](https://en.cppreference.com/w/cpp/types/is_function) in the C++ standard itself looks for to determine if a type qualifies as a function):

    ReturnType [<CallingConvention>] (Args [, ...]) [const] [volatile] [&|&&] [noexcept[(true|false)]]

Note however that the function's "*CallingConvention*" seen above ("*cdecl*", "*stdcall*", "*thiscall*", etc.), isn't part of the C++ standard itself at this writing but it's considered part of a function's type regardless (by all C++ implementations supported by this library). The above templates therefore expect a function in this specific format (for the 2nd template arg "*U*"), and will only return true if the 1st template arg (class or struct type "*T*") has a member function called "*Whatever*" that precisely matches type "*U*". This means that every component of the function seen above is tested for in function "*Whatever*" and must match exactly for these templates to return true (including the return type, calling convention, etc.). In fact, the function does its job by simply calling [std::is_same_v](https://en.cppreference.com/w/cpp/types/is_same), passing the type of "*T::Whatever*" as its 1st template arg and type "*U*" as the 2nd template arg. The comparison therefore looks for an exact match so caution advised. In the above example for instance, if the return type of "*Test::Whatever*" is changed from "*int*" to, say, "*char*", then the call to "*HasNonOverloadedMemberFunction_Whatever_v*" will no longer return true. It will now return false unless you also change "*int*" to "*char*" in its 2nd template arg (so that the return types match again). Note that even the calling conventions of both functions must match or false will be returned.

**IMPORTANT:**

Please note that if the function you're targeting ("*Whatever*" in the above example) is overloaded, then the templates will always return false, even if one of the overloads match the 2nd template arg you're passing to these templates. IOW, the templates are designed to only identify functions that are *not* overloaded. This is because current versions of C++ at this writing do not support any way to target functions that *are* overloaded. The language does not support this (the ability to determine the exact type of a particular overloaded function). While you can target a particular overload in C++ via a "*static_assert*" like so (using the above example to demonstrate this) ...

    // use "static_cast" to target a particular overload
    auto pWhateverOverload = static_cast<int (Test::*)(double) const noexcept>(&Test::Whatever);

... which targets an overload taking a "*double*" instead of a "*float*" (if we added such an overload), the resulting pointer can't be reliably used to determine if the actual "*Whatever*" overload taking a double (if it exists), precisely matches the type being passed to "*static_assert*", namely "*int (Test::*)(double) const noexcept*". That is, if class "*Test*" has, say, an overloaded member function of type "int (Test::*)(double) const", which is identical to the function being passed to "*static_assert*" above but the "*noexcept*" specifier has been removed, then the "*static_cast*" will still compile successfully (normally), but "*pWhateverOverload*" above still (erroneously) thinks it's targeting a "*noexcept*" function (because that's the type that "*pWhateverOverload*" points to due to the type being passed to "*static_cast*"). "*Test::Whatever*" is *not* "noexcept" however so the comparison of "*pWhateverOverload*" (after removing its pointer) to "*int (double) const noexcept*" in our templates will erroneously return true (when in fact this overload of "*Whatever*" is not "*noexcept*"). There is no way to circumvent this issue since the language provides no way to retrieve the precise/accurate signature of any particular overload. This is why these templates can identify non-overloaded functions only (and hence contain the name "*NonOverloaded*" in the template name itself to emphasize this), nor can a template be written that can reliably identify an overloaded function. The templates declared by this macro therefore always return false if the function being targeted is overloaded. For overloaded functions, the templates have no way to reliably retrieve the precise type of the function being targeted because the language doesn't provide any mechanism to do so (at this writing).
</details>

<a name="DECLARE_HAS_NON_OVERLOADED_MEMBER_FUNCTION_TRAITS"></a><details><summary>DECLARE_HAS_NON_OVERLOADED_MEMBER_FUNCTION_TRAITS</summary>
```C++
DECLARE_HAS_NON_OVERLOADED_MEMBER_FUNCTION_TRAITS(NAME)
```
This macro and its cousin [DECLARE_HAS_NON_OVERLOADED_MEMBER_FUNCTION](declare_has_non_overloded_member_function) just above allow you to determine if a class or struct has a non-overloaded, non-static member function with a given name and function type (signature). Additional details are discussed under [DECLARE_HAS_NON_OVERLOADED_MEMBER_FUNCTION](declare_has_non_overloded_member_function) and you should consult this before proceeding further (a comparative summary of these two macros and the templates they create can also be found [here](#determiningifamemberfunctionexists).

As described under [DECLARE_HAS_NON_OVERLOADED_MEMBER_FUNCTION](#declare_has_non_overloded_member_function), the latter macro creates templates used to perform an *exact* match of the function you're checking for with the function signature you pass (say, "*int (std::string_view, float) const noexcept*". The following macro however, DECLARE_HAS_NON_OVERLOADED_MEMBER_FUNCTION_TRAITS, creates templates allowing you to perform a customized match instead (so instead of passing a function type to compare with, as required by the [DECLARE_HAS_NON_OVERLOADED_MEMBER_FUNCTION](#declare_has_non_overloded_member_function) templates), you can effectively pass any criteria required, typically to ignore certain components of the function like its return type, its *noexcept* specification, etc.. You can even target a particular parameter only if you don't need to check them all. You have complete control ove the matching process. Therefore, choose [DECLARE_HAS_NON_OVERLOADED_MEMBER_FUNCTION](#declare_has_non_overloaded_member_function) if you wish to check if a non-overloaded, non-static function precisely matches the exact (entire) signature you pass, or DECLARE_HAS_NON_OVERLOADED_MEMBER_FUNCTION_TRAITS if you need more fine-grained control.

The DECLARE_HAS_NON_OVERLOADED_MEMBER_FUNCTION_TRAITS macro is called the same way as [DECLARE_HAS_NON_OVERLOADED_MEMBER_FUNCTION](#declare_has_non_overloded_member_function). You simply pass the name of the non-static member function you're interested in via the macro's NAME arg ("*Whatever*" in the following example). You'll declare this at namespace scope:

    DECLARE_HAS_NON_OVERLOADED_MEMBER_FUNCTION_TRAITS(Whatever)

This declares the following two templates, where the name of each template always starts with "HasNonOverloadedMemberFunctionTraits_" followed by the NAME arg passed to the macro (representing the name of the member function you're targeting, "*Whatever*" in this case). Note that the 1st template inherits from "*std::true_type*" or "*std::false_type*" as described below (the template arg for this is ommitted in the declaration below since the implementation details aren't relevant). The 2nd template is just a (bool) helper variable template with the usual "_v" suffix that defers to the 1st template in the usual C++ way:
...
... then a template to check for every trait in "*Whatever*" above would look like the following (but if checking *every* trait then you can just rely on [DECLARE_HAS_NON_OVERLOADED_MEMBER_FUNCTION](#declare_has_non_overloded_member_function) instead - that's its purpose and it's both easier and cleaner):

    struct HasAllTraits : std::bool_constant<HasReturnType_v<F, int> &&
                                             HasCallingConvention_v<F, CallingConvention::Cdecl> &&
                                             HasArgTypes_v<F, std::string_view, float> &&
                                             IsVariadic_v<F> &&
                                             IsMemberFunctionConst_v<F> &&
                                             IsMemberFunctionVolatile_v<F> &&
                                             MemberFunctionRefQualifier_v<F> == RefQualifier::None &&
                                             IsNoexcept_v<F>
                                            >
    {
    };
...
```
Default calling convention of a free function if the "*IsFreeFuncT*" template arg is true (which includes static member functions), or a non-static member function otherwise. This refers to the calling convention of a function when not explicitly specified by the user (developer). Returned as a "*CallingConvention*" enumerator (declared in "*TypeTraits.h*"). Calling convention enumerators at this writing include "*Cdecl*", "*Stdcall*", "*Fastcall*", "*Vectorcall*", "*Thiscall*" (applicable to non-static member functions only) and "*Regcall*". Note that in all cases only those calling conventions that are supported by a given compiler can be returned by "*DefaultCallingConvention_v*" (so for instance, "*Vectorcall*" will never be returned on GCC since it doesn't support this calling convention).</details>
...
```
"*bool*" variable set to "*true*" if the (zero-based) "*Ith*" arg of function "*F*" is the same as the given type "*T*" or false otherwise. This template is just a thin wrapper around [std::is_same_v](https://en.cppreference.com/w/cpp/types/is_same), where the latter template is passed the type of the "*Ith*" arg in function "*F*" and type "*T*". It therefore simply compares the type of the "*Ith*" arg in function "*F*" with type "*T*". This is a common requirement for many users (possibly in conjunction with [IsReturnTypeSame_v](isreturntypesame_v)), so this template provides a convenient wrapper. Note that if "*I*" is greater than or equal to the number of args in "*F*" (see [ArgCount_v](#argcount_v)), then a "*static_assert*" will trigger (so if "*F*" has no non-variadic args whatsoever, a "*static_assert*" will always trigger, even if passing zero).</details>
